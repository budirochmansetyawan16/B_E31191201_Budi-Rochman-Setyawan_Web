...
class Grup extends CI_Controller {
	...
	public function index(){
		$data['admin'] = $this->Mahasiswa_model->getAllGrup()->result();
		$this->template->views('crud/master_grup',$data);
	}
	public function edit_grup($id){
		$where = array('id_grup'=>$id);
		$data['admin'] = $this->Mahasiswa_model->edit_data_grup($where,'tm_grup')->result();
	...
	public function update_grup(){
		...
		$this->Mahasiswa_model->update_data_grup($where,$data,'tm_grup');
		redirect('Grup');
	}

	public function hapus_grup($id){
		$where = array('id_grup' => $id);
		$this->Mahasiswa_model->hapus_data_grup($where,'tm_grup');
		redirect('Grup');
	}
}